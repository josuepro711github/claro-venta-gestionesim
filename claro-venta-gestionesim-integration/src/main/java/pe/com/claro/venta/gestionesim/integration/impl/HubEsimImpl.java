...
public class HubEsimImpl implements HubEsim {
	...
	public DownloadOrderResponse descargarPedido(String trazabilidad,
		...
		try {
			...
			elkLegadoBean.setTipoMensaje(ELKUtil.TIPO_MENSAJE_RESPONSE);
			elkLegadoBean.setTiempoRespuesta(System.currentTimeMillis() - tiempoInicio);
			elkLegadoBean.setMensaje(GestionEsimUtil.removerSaltosLinea(String.join("", ExceptionUtils.getRootCauseStackTrace(e)), "\\n"));
			elkLegadoBean.setTipoId(ELKUtil.TIPO_ID_IDT);
			elkLegadoBean.setIdfIdtCodigo(codigoError);
			elkLegadoBean.setIdfIdtMensaje(GestionEsimUtil.removerSaltosLinea(GestionEsimUtil.removerSaltosLinea(e.getMessage(), Constantes.SEPARADOR_ESPACIO)));
